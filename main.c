/*
* Hash Database Creator program - By Lucas Saragosa
...
#include <string.h>
#include <stdlib.h>
//I am using Eteran's 'c-vector' (macro) vector dynamic array - credit to him for that :D
#include "vector.h"
...
const int MAGIC = 0x54544c42;
const int64 crc_tab[];
const char* NEW = "NEWPAGE";

int64 crc(const char* str) {
	int64 crc = 0;
	int len = strlen(str);
	for (int i = 0; i < len; i++) {
...
void wint(FILE* outp, int64 val, int8 bytes) {
	...
	for (int i = 0; i < bytes; i++) {
		buffer[i] = (int8)((int64)val >> ((int64)i << (int64)3));
	}
	fwrite(buffer, 1, bytes, outp);
	free(buffer);
}

char* substr(char* str, int newlen) {
...
typedef struct _page {
	char* name;
	cvector_vector_type(char*) values;
	int size;
} page;
...
int main(int argn, char** argv) {
	char* in1 ,*out1;
	if (argn < 3) {
		printf("Invalid usage. Usage: HashDBCreator.exe \"c:/path/to/my/input/file.txt\" \"c:/path/to/output/db.HashDB\" [-v]\n");
		return 1;
	}
	in1 = argv[1];
	out1 = argv[2];
	...
	fopen_s(&out,out1, "wb");
	if (!in || !out) {
		printf("Could not open the input or output file :(");
		return 1;
	}
	wint(out, MAGIC, 4);
	wint(out, 1, 1);
	char* buf = NULL, *tmp = NULL;
	cvector_vector_type(page*) pages = NULL;
	page* curr = NULL;
	for (;;) {
		if (buf) free(buf);
		buf = (char*)malloc(512);
		if (!buf)return 2;
		buf = fgets(buf, 512, in);
		if (!buf)break;
		//if (feof(in))break;
		if (strlen(buf) >= 8 && !(_stricmp((tmp = substr(buf,7)),NEW))) {
			free(tmp);
			char* pagen =_strdup(buf);
			pagen += 8;
			if (curr) {
				curr = (page*)calloc(1,sizeof(page));
				cvector_push_back(pages, curr);
			}
			else {
				curr = (page*)calloc(1,sizeof(page));
				cvector_push_back(pages, curr);
			...
			if (!curr)return 3;
			curr->name = pagen;
			if (verbose)printf("-new page %s",pagen);
			continue;
		}
		if (!curr)continue;
		char* str = _strdup(buf);
		str[strlen(str) - 1] = 0;
		cvector_push_back(curr->values, str);
		curr->size += strlen(str) + 13;
	}
	if (verbose)printf("-read all values.. (%d pages)\n", cvector_size(pages));
	if (buf)free(buf);
	wint(out, cvector_size(pages), 4);
	for (size_t i = 0; i < cvector_size(pages); i++) {
		page* page = pages[i];
		wint(out, cvector_size(page->values), 4);
		wint(out, 0, 1);
		page->name[strlen(page->name) - 1] = 0;
		wint(out, strlen(page->name), 4);
		fwrite(page->name, strlen(page->name), 1, out);
		...
		for (size_t j = 0; j < cvector_size(page->values); j++) {
			fwrite(page->values[j], strlen(page->values[j]) + 1, 1, out);
		}
		if (verbose)printf("-serialized page %s\n", page->name);
	}
	cvector_free(pages);
	fclose(out);
	fclose(in);
